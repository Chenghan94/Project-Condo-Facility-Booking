<?php
// Project/public/booking.php

require_once __DIR__ . '/../config/db.php';
require_once __DIR__ . '/../src/Facility.php';
require_once __DIR__ . '/../src/Resident.php';
require_once __DIR__ . '/../src/Booking.php';
require_once __DIR__ . '/../src/bootstrap.php';

if (!is_readable(__DIR__ . '/../src/Resident.php')) { die('Resident.php not readable'); }
if (!class_exists('Resident')) { die('Resident class still not loaded'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $date = $_POST['date'] ?? '';
    $slot = $_POST['slot'] ?? ''; // "HH:MM:SS|HH:MM:SS"
    ...
    if (!$unit || !$name || !$email || !$contact || !$facility_id || !$date || !$slot) {
        ...
        if (!$facility) {
            ...
            if ($slot_start < $facility['open_time'] || $slot_end > $facility['close_time']) {
                $err = 'Selected slot is outside operating hours.';
            } else {
                ...
                if ($res['ok']) $msg = 'Booking created successfully! ID: ' . $res['booking_id'];
                else $err = $res['error'];
            }
        }
    }
...
<form method="post">
  ...
  <fieldset>
    ...
    <select name="facility_id" required id="facility">
    ...
    <select name="slot" required id="slot-select">
...
<script>
// Build slot options on the client based on facility open/close/slot_minutes
